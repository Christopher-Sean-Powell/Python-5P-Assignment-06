...
def top_30_combined (w1, book1, s1, book2, d1, book3):
    '''This function is rather complicated, and only very useful for the program word_count. It takes as input three books (or strings), as well as 3 lists of tuples that contain the top 30 words that appear in each of the books. It then combines these top 30 lists, and creates a new list (without repeats). Then, it searches through all 3 books for the number of times each word in the new list appears, and sums those numbers. Then it inputs this informaiton into a new dictionary, and divides the numbers by the total number of words in all 3 books, hence finding the percentages associated with the occurence of each word. The output is in the form of a dicitonary.'''
...
print top_30_combined(wells_30_freq, wells, stevenson_30_freq, stevenson, dickens_30_freq, dickens)
